...
- Attributes:
  - `id` (int) - unique identifier
  - `books` (List<Book>) - collection of books in the order
...
**Method: displayAllAuthors(List<AuthorDTO> authors)**
...
**Method: displayAllBooks(List<BookDTO> books)**
...
**Method: displayBooksByAuthor(List<BookDTO> books, String authorName)**
...
**Method: displayAllOrders(List<OrderDTO> orders)**
...
**Method: createOrder(List<Integer> bookIds)**
...
- Create List<BookDTO> for OrderDTO
- Include total price
...
**Setup Phase:**
1. Instantiate all repositories (AuthorRepository, BookRepository, OrderRepository)
2. Instantiate all views (AuthorView, BookView, OrderView)
3. Instantiate all controllers with their dependencies
...
1. **Start with Model layer** - establish your data structures first
2. **Keep layers independent** - avoid circular dependencies
3. **Use meaningful names** - class and method names should be self-explanatory
4. **Test incrementally** - verify each layer works before moving to the next
5. **Think about data flow** - trace how data moves from Model → Controller → View
6. **Keep Views simple** - only formatting and output, no logic
7. **Controllers coordinate** - they orchestrate operations but don't do the work themselves
